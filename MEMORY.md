...
- 文件：~/.workbuddy/DIALOGUE-LOG.md。实际格式已演变为**两台设备各写一个扁平块**：`## YYYY-MM-DD · 苹果` 与 `## YYYY-MM-DD · 惠普`，互不影响、不嵌套子块（原设计的 `### 🍎`/`### 💻` 子块方案未落地，因自动化 prompt 对子块定位脆弱）
- 铁律：每台设备只写自己的 `· 设备` 块、只读不碰对方块；先 pull 再改再 push
- 错峰运行避免抢写：惠普 17:00，苹果 10:00/17:30（苹果晚于惠普以便拉到其当天块）
- 苹果侧因平台 RRULE 多时间点(`BYHOUR=10,18`)会被重新序列化成单点，故拆成两个自动化分别定 10:00 与 17:30，跑同一合并流程，确保每天两次
- 注：Norman 一般 18:00 已下班锁屏，故苹果第二点定 17:30（早于锁屏、晚于惠普17:00）
