...
for file in "${SOUL_FILES[@]}"; do
    ...
    if [ -f "$dest" ]; then
        ...
        if [ "$local_content" = "$remote_content" ]; then
            continue   # 本地与远程一致，无需操作
        fi
        # 本地与远程不同 => 本地存在未推送的改动，保留本地，绝不覆盖
        continue
    fi
    # 仅当本地文件不存在时才拉取（首次新建）
    echo "$remote_content" > "$dest" && echo "  已拉取: $file (GitHub → 本地，新建)" && PULLED=true
done
...
if [ "$SKILLS_HTTP" -gt 0 ]; then
    # 非破坏性：仅当本地 skills/ 不存在时才拉取，避免覆盖本地未推送的改动
...
if [ "$DS_HTTP" -gt 0 ]; then
    # 非破坏性：仅当本地 daily-summaries/ 不存在时才拉取，避免覆盖本地未推送的改动
